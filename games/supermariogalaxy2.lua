...
local compute = {
  ...
  stageTime = function()
    local address = 0x80A75D10
    values.stageTimeFrames = readIntBE(address + values.o, 4)
  end,
  
  fileTime = function()
    local address = 0x80E40E4C
    
    -- This is a weird combination of big endian and little endian, it seems.
    local lowPart = readIntBE(address + values.o, 2)
    local highPart = readIntBE(address + values.o + 2, 2)
    values.fileTimeFrames = (highPart * 65536) + lowPart
  end,
  ...
  refPointer = function()
    ...
    local pointer2Address = 0x80C7A2C8
  
    -- The pointer.
    local pointer1Address = readIntBE(pointer2Address + values.o, 4) + 0x750
    values.refPointer = readIntBE(pointer1Address + values.o, 4)
  end,
  
  position = function()
    local posStartAddress = values.o + values.refPointer - 0x8670
  ...
  velocity = function()
    ...
    local velStartAddress = values.o + values.refPointer - 0x8C20
...
local getStr = {
  ...
  fileTime = function()
    local centis = math.floor((values.fileTimeFrames % 60) * (100/60))
    local secs = math.floor(values.fileTimeFrames / 60) % 60
    local mins = math.floor(math.floor(values.fileTimeFrames / 60) / 60) % 60
    local hrs = math.floor(math.floor(math.floor(values.fileTimeFrames / 60) / 60) / 60)
    local fileTimeStr = string.format("%d:%02d:%02d.%02d",
      hrs, mins, secs, centis
    )
    local fileTimeDisplay = string.format("File time: %s", fileTimeStr)
    return fileTimeDisplay
  end,
